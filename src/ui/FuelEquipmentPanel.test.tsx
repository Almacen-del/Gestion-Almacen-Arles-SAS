...
it('shows missing mileage without zero and filters real deliveries by date',()=>{
 const m={id:'1',modulo:'Combustible',tipo:'Salida',codigo:'GAS1',descripcion:'Gasolina',referencia:'',cantidad:3,unidad:'GALON',fecha:'2026-09-25T12:00:00Z',solicitante:'Persona',cargo:'',usuario:'',observaciones:'',fotoUrl:'',maquinaria:'moto',placaSerial:'21G',horometro:''} satisfies Movement;
 render(<FuelEquipmentPanel movements={[m,{...m,id:'historic',fecha:'2026-09-24T12:00:00Z',cantidad:12},{...m,id:'old',placaSerial:'87G',cantidad:90},{...m,id:'gray',maquinaria:'camioneta',placaSerial:'875',cantidad:90}]}/>);
 ...
 fireEvent.input(screen.getByLabelText('Maquinaria desde'),{target:{value:'2026-09-26'}});
 ...
 fireEvent.change(screen.getByLabelText('Periodo de control'),{target:{value:'history'}});
 expect(screen.getAllByText('12 gal')[0]).toBeTruthy();expect(screen.getByText(/Fuera del nuevo control/)).toBeTruthy();expect(screen.queryByText('Sin lectura',{exact:true})).toBeNull();
