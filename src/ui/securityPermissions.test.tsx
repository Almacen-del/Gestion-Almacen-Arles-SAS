...
describe('permisos visibles de asignación de lotes',()=>{
 it.each([false,true])('conserva lectura y habilita la asignación solo con permiso: %s',canRegister=>{
  ...
  const html=renderToStaticMarkup(<AgrochemicalExpirationModal registrationScope="demo:warehouse" canRegister={canRegister} products={[{id:'p1',code:'P1',name:'Producto de prueba',stock:10,unit:'GRAMO',location:'COP'}]} lots={[]} entries={[entry]} loading={false} sourceError="" onRegister={register} onClose={vi.fn()} />);
