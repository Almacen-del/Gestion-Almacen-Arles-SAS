...
import { climateLabels, climateRange, colombiaDateTime, criteriaForReading, evaluateClimateParameter, type ClimateMetric, type ClimateCriteria, type ClimateDashboard, type ClimateReading, type ClimateRule } from '../backend/supabase/climate';
...
function ParameterConditions({ criteria, value, metric, code = '' }: { criteria: ClimateCriteria; value: number; metric: ClimateMetric; code?: string }) {
  const evaluated = criteria.rules.filter(r => !code || r.code === code).map(rule => ({ rule, ...evaluateClimateParameter(rule, value, criteria.reference, metric) }));
  const order = { outside: 0, unknown: 1, reference: 2, within: 3 };
  return <section aria-label={metric === 'temperature' ? 'Evaluación de temperatura' : 'Evaluación de humedad'}><h3>{metric === 'temperature' ? 'Temperatura' : 'Humedad relativa'} · {number(value)} {metric === 'temperature' ? '°C' : '%'}</h3><div className="climate-summary" aria-live="polite">{Object.entries(climateLabels).map(([status, label]) => <div key={status} className={`climate-status ${status}`}><strong>{evaluated.filter(r => r.status === status).length}</strong><span>{label}</span></div>)}</div>
    <div className="climate-products">{evaluated.sort((a, b) => order[a.status] - order[b.status] || a.rule.name.localeCompare(b.rule.name)).map(({ rule, status, reasons }) => <details className={`climate-product ${status}`} key={rule.code}>
      ...
      <p><b>Ficha:</b> {metric === 'temperature' ? rule.temperature_text : rule.humidity_text}</p><p>{rule.details}</p>
...
function ProductConditions({ criteria, t, h, code = '' }: { criteria: ClimateCriteria; t: number; h: number; code?: string }) {
  return <div className="climate-separated"><ParameterConditions criteria={criteria} value={t} metric="temperature" code={code} /><ParameterConditions criteria={criteria} value={h} metric="humidity" code={code} /></div>;
}
...
function ProductHistoryChart({ readings, dashboard, code, metric }: { readings: ClimateReading[]; dashboard: ClimateDashboard; code: string; metric: ClimateMetric }) {
  if (!readings.length) return null;
  const colors = { outside: '#c74438', within: '#26864c', reference: '#4b85ac', unknown: '#c29432' };
  return <section className="climate-chart climate-product-history"><h3>{metric === 'temperature' ? 'Evaluación de temperatura' : 'Evaluación de humedad'} · {code || 'Todos los productos'}</h3><div className="climate-legend">{Object.entries(colors).map(([status, color]) => <span key={status}><i style={{ background: color }} />{climateLabels[status as keyof typeof climateLabels]}</span>)}</div>
    <div className="climate-bars">{readings.map(r => {
      const c = criteriaForReading(r, dashboard);
      const states = c?.rules.filter(item => !code || item.code === code).map(item => evaluateClimateParameter(item, Number(metric === 'temperature' ? r.temperature_c : r.humidity_percent), c.reference, metric).status) ?? [];
...
export default function AgrochemicalClimateModal({ onClose }: { onClose: () => void }) {
  ...
  return <div className="modal-backdrop"><section ref={dialog} tabIndex={-1} className="agro-expiration-modal climate-modal" role="dialog" aria-modal="true" aria-labelledby="climate-title" onKeyDown={event => {
    ...
    <header className="evidence-header"><div><p className="eyebrow">Agroquímicos · Bodega Azul</p><h2 id="climate-title"><Thermometer size={22} /> Temperatura y humedad</h2><small>Mediciones desde el celular · Evaluaciones independientes · Hora de Colombia</small></div><button type="button" className="icon-button" aria-label="Cerrar" onClick={onClose}><X size={18} /></button></header>
    <div className="agro-expiration-body climate-body">
      {error && <p role="alert" className="climate-error">{error}</p>}
